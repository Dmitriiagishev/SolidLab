...
```
1. Анализ уязвимости
    * Тип: Reflected XSS (Cross-Site Scripting)
    * Вектор: Параметры show и sort_responses в query string POST-запроса
    * POST с пустым телом (content-length: 0) → фактически данные передаются через URL
    * Пейлоады: Векторы с уникальными идентификаторами (unique_*) для обнаружения отражения в ответах
2. Валидация
   \
    Необходимо определить, попадают ли параметры show/sort_responses в HTML-ответ без должного экранирования? Если параметр используется только для внутренней логики (сортировка, фильтрация на бэкенде) и не рендерится в ответе — это ложное срабатывание.
    * Если в ответе отсутствуют отражение простых уникальных строк из запросы - вероятно, ложное срабатывание
        ```
        # если в ответе нет строки test_xss_12345 - ложное срабатывание
        curl -s -X POST "https://www.target.ru/cat/detail/kovsh-tefal-l6502/page/1/?show=test_xss_12345&sort_responses=date"
        ```
    * Если отражение есть — проверяем, в каком контексте и как экранируется ввод
        ```
        # Проверка на отсутствие экранирования в HTML-теле
        curl -s -X POST "https://www.target.ru/.../?show=<b>XSS_TEST</b>" | grep -i "<b>XSS_TEST</b>"

        # Проверка на экранирование в атрибутах
        curl -s -X POST "https://www.target.ru/.../?show=\"onerror=alert(1)\"" | grep -i "onerror"

        # Проверка на экранирование в JS-контексте
        curl -s -X POST "https://www.target.ru/.../?show=';alert(1);//" | grep -i "alert(1)"

        ```
    * Если экранирование отсутствует - можно создать простой PoC эксплойт
        ```
        <!-- Простой PoC для отражения в HTML-теле -->
        https://www.target.ru/.../?show=<svg/onload=alert(document.domain)>

        <!-- Если в атрибуте -->
        https://www.target.ru/.../?show="onmouseover="alert(1)

        <!-- Если в JS-переменной -->
        https://www.target.ru/.../?show=';alert(1);//
        ```
3. Ликвидациия
    \
    При подтверждении уязвимости ввести меры:
